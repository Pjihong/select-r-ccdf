#' Select Optimal r for r-Largest GEV Model (RGEV11)
#'
#' Sequentially tests goodness-of-fit to select the optimal number r of
#' r-largest order statistics under the nonstationary GEV model (RGEV11)
#' with location trend (mu0 + mu1*t) and stationary scale/shape.
#'
#' @param xdat A numeric matrix of r-largest order statistics (n x maxr),
#'   rows = observations, columns = order (1st largest, 2nd largest, ...).
#' @param model Character string; currently \code{'rgev11'}.
#' @param sigL Significance level for the sequential GOF test (default 0.05).
#' @param method GOF test method: \code{"ed"} (energy distance),
#'   \code{"ccdf"}, or \code{"spacing"}.
...
#' \item{mle}{MLE parameter vector (mu0, mu1, sigma0, sigma1, kappa).}
#' \item{pval}{p-values from sequential GOF tests.}
#' \item{result}{Full result matrix for all tested r values.}
#'
#' @examples
#' \dontrun{
#' # Generate synthetic data and select r
#' para <- c(0, 0.1, 1, 0.02, -0.1)
#' xdat <- ran.rgev11(para, nsample = 30, rlarg = 8)$xdat
#' rsel.rgev11(xdat, method = "ed", sigL = 0.05)
#' }
#'
#' @seealso \code{\link{rsel.rgev}}, \code{\link{rgev11.fit.park}}
#' @export
